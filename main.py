...
def run(args):
    ...
    '''
    values=data_utils.get_stocks(HtmlDownloader())
    writer=open('prices.txt','a')
    if args.t==0:
        stock_strategy(0,values,writer)  # 输出策略
    elif args.t==1:
        while True:
            stock_strategy(date_utils.is_weekend(),values,writer)
    elif args.t==2:
...
def stock_strategy(args,values,writer):
    ...
    if(args==0):#
        ...
        for value in values:
            #关注上下波动超过5个点的股票
            writer.write(' '.join([value.time,value.code,value.now])+'\n')
